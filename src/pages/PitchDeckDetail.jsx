...
export default function PitchDeckDetail({ projectId: projectIdProp = null, embedded = false, onBack = null, onEdit = null }) {
  ...
  if (!projectId || error || !data?.project) return <div className={`${embedded ? 'min-h-[calc(100vh-3.5rem)]' : 'min-h-screen'}  bg-zinc-950 p-8 text-white`}>{embedded ? <button type="button" onClick={onBack} className="inline-flex items-center gap-2 text-cyan-400"><ArrowLeft size={18} /> PITCH DECKS</button> : <Link to="/PitchDecks" className="inline-flex items-center gap-2 text-cyan-400"><ArrowLeft size={18} /> PITCH DECKS</Link>}<p className="mt-12 border-l-4 border-cyan-500 bg-zinc-900 p-6">This private pitch deck is unavailable.</p></div>;

  return <div className={`${embedded ? 'min-h-[calc(100vh-3.5rem)]' : 'min-h-screen'}  bg-zinc-950 pb-20 text-white`}>
    <header className="flex flex-wrap items-center justify-between gap-3 border-b border-zinc-800 px-4 py-4"><div className="flex items-center gap-4">{embedded ? <button type="button" onClick={onBack} className="inline-flex items-center gap-2 text-sm font-bold text-cyan-400"><ArrowLeft size={18} /> PITCH DECKS</button> : <><Link to="/PitchDecks" className="inline-flex items-center gap-2 text-sm font-bold text-cyan-400"><ArrowLeft size={18} /> PITCH DECKS</Link><Link to="/Studio?tab=tools" className="text-xs text-zinc-500 hover:text-white">STUDIO TOOLS</Link></>}</div>{embedded ? <button type="button" onClick={onEdit} className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 px-4 py-2 text-xs font-bold"><Pencil size={14} /> EDIT</button> : <Link to={`/PitchDeckEditor?id=${encodeURIComponent(projectId)}`} className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 px-4 py-2 text-xs font-bold"><Pencil size={14} /> EDIT</Link>}</header>
