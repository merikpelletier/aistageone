...
export default function AssetCard({ asset, category, onQuickView }) {
  return (
    <motion.article layout initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="group overflow-hidden rounded-2xl border border-white/10 bg-zinc-900 transition hover:-translate-y-1 hover:border-cyan-400/50">
      <Link to={`/AssetDetail?id=${encodeURIComponent(asset.id)}`} className="block">
      ...
      <div className="p-5">
        <Link to={`/AssetDetail?id=${encodeURIComponent(asset.id)}`}><h2 className="line-clamp-2 text-lg font-black leading-tight hover:text-cyan-300">{asset.title}</h2></Link>
