...
export default function ConfigurableBottomNav({ settings = [] }) {
  ...
  return <nav className="bottom-nav-safe fixed bottom-0 left-0 right-0 z-50 border-t border-black/20 bg-yellow-400 shadow-lg">
    <div className="flex h-14 w-full items-center md:h-16">
      {navItems.map((item) => {
        ...
        const content = <>
          <div className="relative"><Icon size={20} className="text-black" />{item.cart && cartCount > 0 && <span className="absolute -right-2 -top-2 flex h-4 w-4 items-center justify-center rounded-full bg-red-600 text-xs font-bold leading-none text-white">{cartCount}</span>}</div>
          <span className="hidden max-w-full truncate text-[10px] font-semibold tracking-wide text-black md:block">{item.account && !user ? 'LOG IN' : item.label}</span>
          {active && <motion.div layoutId="nav-indicator" className="absolute bottom-1 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full bg-red-600" transition={{ type: 'spring', stiffness: 500, damping: 30 }} />}
        </>;
