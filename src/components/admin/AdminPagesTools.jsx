...
function LookSection({ surface, draft, setDraft }) {
  ...
  return <div className="grid gap-4 md:grid-cols-2">
    ...
    {surface.key === 'bottom_navigation' ? <>
      <div><span className={label}>Icons and text</span><div className="flex gap-2"><input type="color" value={draft.look.foreground_color || '#000000'} onChange={(event) => update('foreground_color', event.target.value)} className="h-11 w-14 rounded-lg bg-transparent" /><input className={field} value={draft.look.foreground_color || ''} onChange={(event) => update('foreground_color', event.target.value)} placeholder="Keep current black" /></div></div>
