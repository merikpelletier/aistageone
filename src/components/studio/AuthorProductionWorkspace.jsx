...
function PanelLeafEditor({ panel, path, direction, ratio, disabled, onUpdate, renderLeaf }) {
  ...
  return <div className="space-y-2">
    <div ref={containerRef} className={`flex ${isVertical ? 'flex-row' : 'flex-col'} gap-0 rounded-xl border border-yellow-400/20 bg-black/30 p-2`}>
      <div className={isVertical ? 'min-w-0' : 'min-h-0'} style={isVertical ? { width: `${currentRatio * 100}%` } : { height: 'auto' }}>
        <PanelLeafEditor panel={panel.split.a} path={[...path, 'a']} disabled={disabled} onUpdate={(nextChild) => onUpdate({ ...panel, split: { ...panel.split, a: nextChild } })} renderLeaf={renderLeaf} />
      </div>
      <button type="button" aria-label="Drag to resize split" onPointerDown={begin} disabled={disabled} className={`shrink-0 bg-yellow-400/70 hover:bg-yellow-400 ${isVertical ? 'mx-1 w-1.5 cursor-col-resize rounded-full' : 'my-1 h-1.5 cursor-row-resize rounded-full'}`} />
      <div className={isVertical ? 'min-w-0 flex-1' : 'min-h-0'} style={!isVertical ? {} : {}}>
        <PanelLeafEditor panel={panel.split.b} path={[...path, 'b']} disabled={disabled} onUpdate={(nextChild) => onUpdate({ ...panel, split: { ...panel.split, b: nextChild } })} renderLeaf={renderLeaf} />
      </div>
    </div>
    <div className="flex gap-2">
...
function AuthorExportModal({ project, activeChapterId, onClose }) {
  ...
  return <div className="fixed inset-0 z-[115] flex items-center justify-center bg-black/85 p-3"><div className="max-h-[94vh] w-full max-w-3xl overflow-y-auto rounded-3xl bg-zinc-950 p-5 text-white shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-widest text-yellow-400">Additional exports</p><h3 className="mt-1 text-2xl font-black">Export Story Block</h3><p className="mt-2 text-sm text-white/55">These downloads never replace or modify the original Story Block.</p></div><button onClick={onClose} disabled={Boolean(task)} className="rounded-full bg-white/10 p-2 text-yellow-400 disabled:opacity-35"><X size={19} /></button></div>
    <div className="mt-5"><p className="mb-2 text-xs font-black uppercase text-white/50">Completed chapters to include</p><div className="space-y-2">{readyChapters.map((chapter) => <button key={chapter.id} onClick={() => toggle(chapter.id)} disabled={Boolean(task)} className={`flex w-full items-center gap-3 rounded-xl border p-3 text-left font-black ${selectedIds.includes(chapter.id) ? 'border-yellow-400 bg-yellow-400/10 text-yellow-300' : 'border-white/10 bg-white/5 text-white/55'}`}><span className="flex h-5 w-5 items-center justify-center rounded border">{selectedIds.includes(chapter.id) && <Check size={14} />}</span><span className="flex-1">Chapter {chapter.number} · {chapter.title}</span><span className="text-xs">9 segments</span></button>)}</div>{!readyChapters.length && <p className="rounded-xl bg-red-500/15 p-3 text-sm font-bold text-red-200">Complete and approve all 9 images and narrations in a chapter before exporting it.</p>}</div>
    <div className="mt-5 grid gap-4 md:grid-cols-2"><section className="rounded-2xl border border-yellow-400/30 bg-yellow-400/5 p-4"><Film className="text-yellow-400" /><h4 className="mt-3 text-lg font-black">Animated video</h4><p className="mt-2 text-sm leading-relaxed text-white/60">Camera movement on each final image, soft transitions and synchronized narration. The MP4 is created locally and downloaded directly.</p><p className="mt-3 text-xs font-black text-green-300">No AI generation · no AISTAGE storage</p>{!mp4Supported && <p className="mt-3 rounded-lg bg-orange-400/15 p-2 text-xs font-bold text-orange-200">Local MP4 recording is not available in this browser. Use the current desktop version of Edge, Chrome or Safari.</p>}<button onClick={() => run('video')} disabled={Boolean(task) || !chapters.length || !mp4Supported} className={`${action} mt-4 w-full bg-yellow-400 text-black`}>{task === 'video' ? <Loader2 size={15} className="animate-spin" /> : <Download size={15} />} Download animated MP4</button><p className="mt-2 text-[11px] text-white/40">Rendering takes approximately the full narration duration. Keep this window open.</p></section>
      <section className="rounded-2xl border border-cyan-300/30 bg-cyan-300/5 p-4"><Images className="text-cyan-300" /><h4 className="mt-3 text-lg font-black">Slideshow / carousel</h4><p className="mt-2 text-sm leading-relaxed text-white/60">Numbered final images for reading, printing or social carousels, plus matching narration files and a Story Block manifest.</p><p className="mt-3 text-xs font-black text-green-300">No AI generation · no AISTAGE storage</p><button onClick={() => run('slides')} disabled={Boolean(task) || !chapters.length} className={`${action} mt-4 w-full bg-cyan-300 text-black`}>{task === 'slides' ? <Loader2 size={15} className="animate-spin" /> : <Download size={15} />} Download slideshow ZIP</button></section></div>
    {(task || progressLabel) && <div className="mt-5 rounded-xl bg-white/5 p-3"><div className="flex justify-between text-xs font-black"><span>{progressLabel}</span><span>{Math.round(progress * 100)}%</span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-white/10"><div className="h-full bg-yellow-400 transition-[width]" style={{ width: `${progress * 100}%` }} /></div></div>}
  </div></div>;
}
