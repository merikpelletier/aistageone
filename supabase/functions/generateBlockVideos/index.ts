...
serveWithCors(async (req) => {
  try {
    ...
    const failSegment = async (segmentIndex, errorMessage) => {
      const currentSegment = segments[segmentIndex] || {};
      const attempts = Number(currentSegment.generation_attempts || 0) + 1;
      ...
      return Response.json({
        ...
        attempt: attempts,
        error: terminal ? `Scene ${segmentIndex + 1} failed after ${attempts} attempts: ${errorMessage}` : undefined,
        message: terminal
          ? `Scene ${segmentIndex + 1} failed after ${attempts} attempts.`
          : `Scene ${segmentIndex + 1} failed. Retrying (${attempts}/${MAX_SEGMENT_ATTEMPTS})...`,
      });
    };
    ...
    if (predictionId) {
      ...
      if (ageMs > FALLBACK_POLL_MS) {
        ...
        if (!pollRes.ok) {
          return await failSegment(nextSegmentIndex, `Replicate polling failed (${pollRes.status})`);
        }
          ...
          if (pollData.status === 'succeeded') {
            ...
            if (mediaUrl) {
              ...
              if (mediaRes.ok) {
                ...
                const updatedSegments = [...segments];
                updatedSegments[nextSegmentIndex] = { ...segment, prediction_id: null, prediction_created_at: null, generation_attempts: 0, last_error: null, redo_note: null };
            ...
            return await failSegment(nextSegmentIndex, errorMsg);
          }
        }
      }
    ...
    if (!predRes.ok) {
      ...
      // Non-rate-limit error: retry this segment a bounded number of times.
    ...
    if (predData.status === 'succeeded') {
      ...
      return await failSegment(
        nextSegmentIndex,
        mediaUrl ? 'Unable to download generated media' : 'Replicate returned no media URL',
      );
    }

    if (!predData.id) {
      return await failSegment(nextSegmentIndex, 'Replicate returned no prediction ID');
    }
