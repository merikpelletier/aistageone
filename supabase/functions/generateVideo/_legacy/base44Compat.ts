...
import { generateSpeech, generateText, transcribeAudio } from './openai.ts';
...
async function processAgentMessage(
  ...
  const answer = await generateText({
    instructions: agentPrompts[conversation.agent_name] || agentPrompts.production_assistant,
    prompt,
    imageUrls: Array.isArray(message.file_urls) ? message.file_urls as string[] : [],
    reasoningEffort: isStoryAgent ? 'medium' : 'low',
  });
  messages.push({
    id: crypto.randomUUID(),
    role: 'assistant',
...
export function createClientFromRequest(request: Request) {
  ...
  const integrations = {
    Core: {
      ...
      GenerateSpeech: async (payload: { text: string; voice?: string }) => {
