...
Deno.serve(async (req) => {
  try {
    ...
    if (session.simulation_run_id) {
      ...
      const allSimChars = await base44.entities.SimCharacter.filter({ session_id, simulation_run_id: runId });
      ...
      if (heroSim) {
        ...
        let heroLoc = heroLocId ? await base44.entities.SimLocation.get(heroLocId).catch(() => null) : null;
        ...
        const allSimAssets = heroLocId ? await base44.entities.SimAsset.filter({ session_id, simulation_run_id: runId }) : [];
        ...
        const allSits = await base44.entities.SimActiveSituation.filter({ session_id, simulation_run_id: runId });
        const heroSits = allSits.filter(s => (s.affected_character_ids || []).includes(heroSim.id));

        const allObs = await base44.entities.SimObservation.filter({ session_id, simulation_run_id: runId });
        ...
        const allBeliefs = await base44.entities.SimBelief.filter({ session_id, simulation_run_id: runId });
        ...
        const allTrans = await base44.entities.SimInformationTransmission.filter({ session_id, simulation_run_id: runId });
        ...
        const allClaims = heroTransClaimIds.size ? await base44.entities.SimClaim.filter({ session_id, simulation_run_id: runId }) : [];
        ...
        const wt = (await base44.entities.SimWorldTime.filter({ session_id, simulation_run_id: runId }))[0];
...
Use this Hero-scene context TOGETHER WITH the previous chapter ending and the user's selected action below. It reflects ONLY what the Hero currently knows/perceives — do NOT reveal information the Hero does not have (other characters' private beliefs, offscreen events, or anything not listed here). Keep the scene consistent with who is present, the environment, and the Hero's knowledge. You are NOT asked to change simulation records — only to narrate consistently with them. CAST CONTROL: use ONLY the ACTIVE CHAPTER CAST for named actions and dialogue. Background extras are unnamed crowd — give them NO dialogue, goals, beliefs, or continuity. Deferred relevant characters must NOT appear as named actors this chapter unless a justified transition is written; they return in later chapters.`;
    ...
    const charsDescription = supportingChars.map(c =>
      `- ${c.name || 'Unnamed'} (ID: ${c.id}, type: ${c.character_type || 'character'}): ${c.description || 'N/A'} | Traits: ${(c.traits || []).join(', ') || 'N/A'}${c.photos?.length ? ` | Photos: ${c.photos.join(', ')}` : ''} | Backstory: FETCH via StoryCharacter read tool using ID ${c.id} (not included inline).`
    ).join('\n');
...
NOTE: Full backstories for supporting characters are intentionally NOT included here (kept out to avoid message-size limits). You have read access to the StoryCharacter entity. For any supporting character you decide to feature in this block, use the StoryCharacter read tool with that character's ID (provided above) to fetch their full backstory BEFORE writing the scene, so you can portray them accurately. The Hero's full backstory IS provided inline below.
