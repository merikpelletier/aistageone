...
Deno.serve(async (req) => {
  try {
    ...
    const { session_id, block_ids, title: customTitle, description: customDesc, category: customCategory, author_name: customAuthor, cover_image: customCover, dossier_class: customClass } = await req.json();
    if (!session_id) {
    ...
    const allBlocks = await base44.entities.StoryBlock.filter({ session_id }, 'order', 200);
    ...
    try {
      const theme = await base44.entities.StoryTheme.get(session.theme_id);
      if (theme) {
    ...
    try {
      const hero = await base44.entities.StoryCharacter.get(session.hero_story_character_id);
      if (hero?.name) {
    ...
    const existingPages = await base44.entities.DossierPage.filter({ block_player_episode_page_id: session_id });
    ...
    await base44.entities.StorySession.update(session_id, {
      is_published: true,
